...
27. Не существует задачи без статуса в проекте. Им автоматически присваивается статус "CREATED"**
...
- Создать статус: Любой участник проекта может создать статус, передав название статуса и цвет для отображения в UI.
- Удалить статус: Любой участник проекта может удалить статус, но только если он не присвоен ни одной задаче.
...
- Назначить исполнителя: Любой участник проекта может назначить другого участника проекта исполнителем задачи.
  Состояния:
- Новосозданная (CREATED): Задача только что создана. Дефолтное значение.
- Также может зависеть от созданных статусов участников проекта.
...
Термины: Статус (Status), Название (Name), Цвет (Color), Порядок (Order), Создать статус (Create Status), Удалить статус (Delete Status).**
...
2. Project Aggregate
   ...
   - Ограниченный контекст: Project Management, Status Management, Task Management
   - Комментарий: Включает в себя логику управления проектами, задачами и статусами, объединяя их в единый контекст.
